...
import { updateGlobalState, getLastUserMessage, type SessionStatus } from '../state/globalState';
...
export async function GET() {
  ...
  return Response.json({ sessions: sessionsWithLastMessage });
}
...
export async function POST(request: Request) {
  ...
  if (!cwd || !sessionId) {
    return Response.json({ error: 'Missing cwd or sessionId' }, { status: 400 });
  }
