...
import { REVIEW_DIR, getReviewFilePath, readJsonFile, writeJsonFile, ensureDir } from '@cockpit/shared-utils';
...
export async function GET() {
  try {
    ...
    if (order.length > 0) {
      ...
      return Response.json({ reviews: [...ordered, ...unordered] });
    }
    reviews.sort((a, b) => b.createdAt - a.createdAt);
    return Response.json({ reviews });
  } catch (error) {
    console.error('Error listing reviews:', error);
    return Response.json({ error: 'Failed to list reviews' }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    if (!title || content === undefined || !sourceFile) {
      return Response.json({ error: 'title, content and sourceFile are required' }, { status: 400 });
    }
    ...
    if (existing) {
      ...
      return Response.json({ review: { id: existing.id, title: existing.title, active: existing.active, createdAt: existing.createdAt, updatedAt: existing.updatedAt, existing: true } });
    }
    ...
    return Response.json({ review: { id, title, active: true, createdAt: review.createdAt, updatedAt: review.updatedAt } });
  } catch (error) {
