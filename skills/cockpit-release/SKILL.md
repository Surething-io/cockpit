...
```bash
# Injection gating, BOTH directions. The negative case is the security-relevant
# one: an un-marked document must NOT carry the live bash bridge.
A=$(curl -fsS "$B/apps/builtin/file-viewer/index.html")
echo "$A" | grep -qi "<html" || FAIL=1
echo "$A" | grep -q  "cockpit.bash" && { echo "SDK leaked into un-marked doc"; FAIL=1; }

Bd=$(curl -fsS "$B/apps/builtin/file-viewer/index.html?bash=1")
echo "$Bd" | grep -q "window.cockpit" || { echo "SDK missing on ?bash=1 doc"; FAIL=1; }
