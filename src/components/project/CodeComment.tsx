  ...
  return (
    <div className="bg-blue-3/50 dark:bg-blue-3/20 border-l-2 border-blue-9 mx-2 my-1 rounded-r">
      <div className="flex items-center px-3 py-1.5 border-b border-blue-6/30">
        <span className="text-xs text-blue-11 font-mono">{t('comments.newComment', { range: lineRange })}</span>
      </div>
      <div className="px-3 py-2 space-y-2">
        <textarea
          ...
          rows={3}
          placeholder={t('comments.commentPlaceholder', { modKey: modKey() })}
        />
        <div className="flex items-center justify-end gap-2">
          <button
            onClick={onCancel}
...
export function CommentIcon({ count, onClick }: CommentIconProps) {
  ...
  return (
    <button
      ...
      title={t('comments.nComments', { count })}
    >
