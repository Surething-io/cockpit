...
| Runs offline | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ |
| Choose between model variants | Fixed (latest) | Fixed | Live list from DeepSeek's API | Live list from GLM's API | Live list from your Kimi plan | Any model you've pulled |
| Shows running cost in the UI | ✅ | — | — | Quota, not cost — see [Check quota](#check-your-coding-plan-quota) | Quota, not cost — see [Check quota](#check-your-quota) | Free |

> Image support is engine-level. **Ollama** tabs **silently drop** image attachments (no error, but the AI doesn't see them). **DeepSeek**, **GLM** and **Kimi** run the Built-in Agent, which is text-only — see [What runs the API-key engines](#what-runs-the-api-key-engines).

### What runs the API-key engines

**Claude** and **Codex** are driven by their vendor's own tooling — the Claude Agent SDK and the `codex` CLI. The three API-key engines (**DeepSeek**, **GLM**, **Kimi**) and **Ollama** all run **Cockpit's own Built-in Agent** against the provider's OpenAI-compatible endpoint. There is no per-tab mode switch: the engine you pick decides what runs.

What the Built-in Agent gives you: streaming replies, tool calls rendered inline, [snapshots](/en/docs/agent/snapshots/), forking, and per-project session files under `~/.cockpit/<engine>-sessions/`.

What it doesn't have — the same list for all three engines, so it isn't repeated per section below:

- **No images.** An images-only message is rejected with *"The built-in agent requires a text prompt"*; images sent alongside text are dropped and the text is answered.
- **Seven tools, no more:** `Read`, `Write`, `Edit`, `Bash`, `Glob`, `Grep`, `TodoWrite`. No subagents, no MCP servers, no web fetch/search.
- **No dollar cost readout.** Token counts show; the USD total stays at zero. Use the provider's own balance / quota button or console.

> **Upgrading?** Earlier Cockpit versions offered a per-tab *Claude Agent SDK / Built-in Agent* toggle for these engines. It's gone, and so is the SDK half. Transcripts written by the old SDK mode under `~/.cockpit/<engine>/projects/` are **untouched on disk** but no longer appear in the sidebar or the Session Browser, since no loop remains that could resume them.
...
- **DeepSeek** — get a key from [platform.deepseek.com](https://platform.deepseek.com/), then **paste it in the DeepSeek picker in the Agent tab header** (not in the global Cockpit Settings). Pick a model in the same picker.
...
Under the hood it goes through DeepSeek's OpenAI-compatible endpoint (`https://api.deepseek.com/v1`), driven by Cockpit's [Built-in Agent](#what-runs-the-api-key-engines) — so tool calls and streaming work, but images, MCP and subagents don't.
...
3. Pick a model in the same picker.

Done. The key only ever stays on your machine.

### Pick a DeepSeek model

The model list is **fetched live** from DeepSeek's `GET /v1/models` using your key — Cockpit whitelists nothing, so a model DeepSeek ships tomorrow shows up on its own (and an id your account can't serve is rejected by DeepSeek with its own message).

A new DeepSeek tab defaults to **`deepseek-v4-pro`**. The `flash` variants are the cheap, fast end of the list — good for quick fixes, formatting and simple Q&A; `pro` is slower and smarter, for architecture decisions, hard bugs and multi-step refactors.

### Check your balance

DeepSeek is prepaid, so the DeepSeek tab gets a **balance** button next to the model picker. It needs a saved key and doesn't poll — it fetches only when you click.

### What you get

- **A model picker**, live from DeepSeek's API (see above).
- Streaming replies.
- Tool use — DeepSeek can read your files, run shell commands, edit code.
- **Forking** and per-tool-call [snapshots](/en/docs/agent/snapshots/), like every other engine.
- Token counts visible in the UI.

### What you don't get

Everything in [What runs the API-key engines](#what-runs-the-api-key-engines) applies: no images, seven tools only, and **no dollar total** — the token bar's USD figure stays at zero. Use the balance button, or check your DeepSeek dashboard for the real bill.
...
- **Slow / hanging replies** — `pro` is genuinely slower than `flash`; if you don't actually need the reasoning, switch the tab to a `flash` model.
- **Spending climbing fast** — `pro` is several times more expensive than `flash`. Click the balance button to see where you stand.
...
> **GLM publishes no per-model metadata.** Its model list is bare ids — no display name, no context window. So the picker shows ids only, and Cockpit sets no context-window hint for GLM tabs (Kimi tabs get one because Kimi reports it). That's a limit of the provider's API rather than a missing Cockpit feature.

> **`glm-5.2[1m]`.** BigModel's Claude Code docs mention that `[1m]` suffix for a 1M-token context. Cockpit used to reject it outright — its old Anthropic-compatible route answered HTTP 400 *"模型不存在"*. GLM tabs now go through the OpenAI-compatible coding endpoint instead, so if you need the long-context variant, try it and let the provider answer; a model id your account can't serve comes back as GLM's own error.
...
| Region | Endpoint |
|---|---|
| **中国大陆** — mainland | `https://open.bigmodel.cn/api/coding/paas/v4` |
...
Everything in [What runs the API-key engines](#what-runs-the-api-key-engines) applies — no images, seven tools, no USD total. GLM-specific on top of that:

- **No dollar cost readout at all.** The token bar's USD figure stays at zero. On a Coding Plan use **Check quota**; on pay-as-you-go check the BigModel console.
...
- **HTTP 400 "模型不存在"** — that model id isn't served to your account. Pick one from the list the picker fetched.
...
- **Pasted images are ignored** — GLM tabs are text-only. Use a Claude or Codex tab when you need the model to see an image.
...
Everything in [What runs the API-key engines](#what-runs-the-api-key-engines) applies — no images, seven tools, no USD total. Kimi-specific on top of that:

- **No dollar cost readout.** Kimi Code bills by subscription, so there's nothing per-token to total up — use **Check quota** instead.
...
- **Old Kimi transcripts are gone from Cockpit.** Sessions used to live in `~/.kimi`, and later in `~/.cockpit/kimi/projects/` (the removed SDK mode); Cockpit indexes neither, so those conversations won't appear in the sidebar or the Session Browser. **The files are untouched on disk** if you want to read or archive them yourself. New sessions land under `~/.cockpit/kimi-sessions/`.
...
- **Pasted images are ignored** — Kimi tabs are text-only. Use a Claude or Codex tab when you need the model to see an image.
