普通的 HTML 预览是**纯静态**的 —— 页面想拉真实数据,会被同源沙箱用 CORS 拦住。**HTML 应用**把这层墙拆了:用 `/html` 让 AI 生成一个 HTML 应用,预览时注入一个 `window.cockpit` SDK —— **本质就是把 Bash 工具暴露给页面**。于是按钮能 `curl` 拉数据、读写文件、跑脚本,一份静态页就成了有后端能力的小工具。
...
- **Console 浏览器气泡**:让 HTML 应用在会话里常驻运行的地方。现在三种预览(聊天弹窗、Explorer、气泡)都带 SDK,气泡只是切走再回来还留着。

### HTML 面板

侧边栏 **应用** 区域的第一个图标会打开 **HTML 应用** 弹窗。它会列出你收藏的 HTML 应用,顶部可以搜索,也可以点 **添加** 手动登记一个已有 `.html` 文件的绝对路径。空列表时,弹窗会提示你用 `/html` 新建,或添加已有 HTML 文件。

每张卡片的标题 / 说明 / 图标来自页面 `<head>`:
...
点卡片会打开预览;悬停卡片时可以点**在 Console 气泡打开**、**删除**,路径旁边的复制按钮会复制文件路径。缩小后的预览会留在侧边栏应用区域里,点对应图标会恢复同一个运行中的预览。文件被删或移走后,卡片会灰显并标 `无效`,只能删除或复制路径,不能打开或预览。

面板只记**绝对路径**,登记表在 `~/.cockpit/html.json`(机制和 `skills.json` 一样)。HTML 文件本身留在你项目里 —— 面板只是个书签夹。手动添加时必须填绝对路径,并且目标文件必须存在、可读、扩展名是 `.html` 或 `.htm`。

### 用 `/名字` 快速打开

在 Console 输入栏打 `/`,已登记的 HTML 应用会**排在自定义命令前面**(带蓝色 `HTML` 标签),选中或回车就在气泡里打开。这个短名就是页面 meta 里的 `cockpit-name`。它不拦真实命令 —— `/usr/bin/x` 仍按路径处理。

## 下一步

- [Skills](/zh/docs/agent/skills/) —— `/html` 所在的 `/` 命令菜单
- [工作流](/zh/docs/agent/workflows/) —— 把多个命令串成有序步骤
- [浏览器气泡](/zh/docs/console/browser/) —— HTML 应用运行的地方
