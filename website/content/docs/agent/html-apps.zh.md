...
左下角侧边栏的 **应用** 打开 **HTML 应用** 弹窗。它会列出你收藏的 HTML 应用,顶部可以搜索,也可以点 **添加** 手动登记一个已有 `.html` 文件的绝对路径。空列表时,弹窗会提示你用 `/html` 新建,或添加已有 HTML 文件。
...
点卡片会在 Console 浏览器气泡里打开;悬停卡片时可以点**预览**、**删除**,路径旁边的复制按钮会复制文件路径。文件被删或移走后,卡片会灰显并标 `无效`,只能删除或复制路径,不能打开或预览。
