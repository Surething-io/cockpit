...
前 7 个是**模式** —— 改变 AI 这一次回复的工作方式,下面每个都有单独一节。后 4 个是一次性任务,指令都在各自的 SKILL.md 里,没有需要配置的东西。
...
> 上面 11 个内置 Skills 就是 Cockpit 带的全部。要做你自己的重复工作流，看下面的[自定义 Skills](#自定义-skills) —— 它们出现在同一个 `/` 菜单里，叫 `/skill-name`。
