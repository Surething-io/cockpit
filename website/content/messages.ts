...
export const messages = {
  en: {
    ...
    panels: {
      ...
      explorer: {
        ...
        bullets: [
          ...
          'Code Map — every function as a chip with caller/callee pins (TS/JS/Python/Go/Rust)',
    ...
    codeMap: {
      ...
      desc: 'Every function becomes a chip with its body, callers on the left, callees on the right. Click a pin to jump — never lose the call graph again.',
      bullets: [
        'Function-level chips with caller / callee pins on both sides',
        'Click any pin to jump straight to the target function',
        'Chip-level diff overlay — see what changed inside each function',
        'Multi-language support: TypeScript / JavaScript, Python, Go, Rust',
        'Built on tree-sitter — no LSP required, works offline',
        'Pluggable LanguageHandler — add your own grammar in ~200 lines',
      ],
  ...
  zh: {
    ...
    panels: {
      ...
      explorer: {
        ...
        bullets: [
          ...
          'LSP 集成 —— 跳转定义、查找引用',
          '代码地图 —— 每个函数渲染为 chip，左右列 caller / callee 即点即跳（TS/JS/Python/Go/Rust）',
    ...
    codeMap: {
      ...
      desc: '每个函数渲染成一个 chip：左侧列 caller、右侧列 callee，函数体居中。点击两侧的 pin 即可跳转 —— 从此不再迷路于调用图。',
      bullets: [
        '函数级 chip 视图，两侧分别列出 caller / callee',
        '点击任意 pin 即可跳转到对应函数',
        'Chip 级 diff 叠加 —— 一眼看出每个函数内部的改动',
        '多语言支持：TypeScript / JavaScript、Python、Go、Rust',
        '基于 tree-sitter —— 不依赖 LSP，离线可用',
        '可插拔 LanguageHandler —— 约 200 行即可接入新语言',
      ],
